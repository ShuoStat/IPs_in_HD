...
for(i in d[c(3, 4, 6)]){
  ...
  plot_predPoint(obj, mstops = mstop, top = 5, ylim = c(0, 14),
                 ref = "mean", name = paste0(gsub(".RData", "", i), "(adaptive)"))
  
  plot_predCorr(obj, mstops = mstop, top = 3, ref = "mean",
                name = paste0(gsub(".RData", "", i), "(fixed)"))
...
for(i in d[-c(3, 4, 6)]){
  ...
  obj <- lapply(mstops$mstops, `[[`, "cvboost")
  mstop <- unlist(lapply(mstops$mstops, `[[`, "mstop"))
  plot_predPoint(obj, mstops = mstop, top = 5, ylim = c(0, 12), ref = "mean",
